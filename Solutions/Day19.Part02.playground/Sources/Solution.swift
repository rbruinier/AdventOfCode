...
func transformInfoForScannerIndex(scannerIndex: Int, from relations: Set<Relation>, scanners: [Scanner], matrices: [Matrix]) -> Point {
...
public func solutionFor(input: Input) -> Int {
    ...
    for scannerIndex in 0 ..< scanners.count {
        let location = transformInfoForScannerIndex(scannerIndex: scannerIndex, from: relations, scanners: scanners, matrices: rotationMatrices)

        scannerLocations.append(location)
    }
