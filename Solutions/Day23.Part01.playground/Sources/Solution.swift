...
public enum Amphipod: Int, Equatable, Hashable, RawRepresentable {
    case none
    case amber
    case bronze
    case copper
...
func possibleMovesFor(spot: Spot, gameState: GameState) -> [Move] {
    switch spot {
...
func executeMove(_ move: Move, on gameState: GameState) -> GameState {
...
func playAllNextPossibleMoves(with gameState: GameState, stateCostCache: inout [String: Int]) -> Int {
    if gameState.isComplete { // \ 0 /
        print("Completed a game with total cost: \(gameState.totalCost)")
