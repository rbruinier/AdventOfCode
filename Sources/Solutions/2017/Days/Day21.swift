...
final class Day21Solver: DaySolver {
    ...
    let expectedPart2Result = 2_766_750
    ...
    func solvePart1() -> Int {
        return solve(tiles: Array(input.tiles), iterations: 5).count
    }
    ...
    func parseInput(rawString: String) {
        ...
        func rotatePattern(_ input: Set<Point2D>, size: Int, degrees: Point2D.Degrees) -> Set<Point2D> {
            Set(input.map { point in
                switch degrees {
                case .zero, .threeSixty:
                    return point
                case .ninety:
                    return point.turned(degrees: .ninety) + .init(x: 0, y: size - 1)
                case .oneEighty:
                    return point.turned(degrees: .oneEighty) + .init(x: size - 1, y: size - 1)
                case .twoSeventy:
                    return point.turned(degrees: .twoSeventy) + .init(x: size - 1, y: 0)
                }
            })
        }
