...
final class Day22Solver: DaySolver {
    ...
    private func cut(deck: [Int], n: Int) -> [Int] {
        if n >= 0 {
            return Array(deck[n ..< deck.count] + deck[0 ..< n])
        } else {
            return Array(deck[deck.count + n ..< deck.count] + deck[0 ..< deck.count + n])
        }
    }
    ...
    func parseInput(rawString: String) {
        ...
        let steps: [Step] = rawString.allLines().map { line in
            if line.starts(with: "deal with increment") {
                return Step.increment(n: Int(line.trimmingCharacters(in: invalidNumberCharacterSet))!)
            } else if line.starts(with: "cut") {
                return Step.cut(n: Int(line.trimmingCharacters(in: invalidNumberCharacterSet))!)
            } else {
                return Step.reverse
            }
        }
